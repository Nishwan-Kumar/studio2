...
export async function getPosts(): Promise<Post[]> {
  try {
    ...
    const postsWithAuthors = await Promise.all(postsList.map(async (post) => {
      ...
      return {
        ...post,
        author: author || { id: 'unknown', name: 'Unknown User', avatarUrl: 'https://placehold.co/100x100.png' },
  ...
  try {
    ...
    if (postDocSnap.exists()) {
      ...
      const commentsWithAuthors = await Promise.all((postData.comments || []).map(async (comment: any) => {
        ...
        return {
          ...comment,
          author: commentAuthor || { id: 'unknown', name: 'Unknown User', avatarUrl: 'https://placehold.co/100x100.png' },
...
export async function getUserPosts(userId: string): Promise<Post[]> {
  ...
  try {
    ...
    // We can assume the author is the same for all posts, so we fetch it once.
